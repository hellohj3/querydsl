...
import study.querydsl.entity.Team.QTeam;
...
import static study.querydsl.entity.Team.QTeam.*;
...
class QuerydslBasicTest {
    ...
    public void join_on_no_relation() {
        ...
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                .leftJoin(team).on(member.username.eq(team.name))   // 조인절에 엔티티 하나만 넣으면 ON 절을 커스텀 사용
                .fetch();
    ...
    public void fetchJoinNo() {
        ...
        boolean loaded = emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam());   // 로딩 된건지 아닌지 판단
    ...
    public void sqlFunction2() {
        ...
        List<String> result = queryFactory
                ...
                /* .where(member.username.eq(Expressions.stringTemplate(
                        "function('lower', {0})",
                        member.username))) */
                .where(member.username.eq(member.username.lower()))     // ansi 표준은 Querydsl 에서 지원해줌
                .fetch();
